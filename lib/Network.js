...
NEAT.Network = function(genome) {
	...
	*/
	...
	/*
	if the network is fully connected, sort the inputs using their id's
	this is done so the input array the user provides always coorisponds 
	to the same nodes across networks
	*/
	if (genome.population.initialConnectivity === 1) {
...
// feeds an input of 1 to the bias node
NEAT.Network.prototype.initBias = function() {

	// for every input node...
	for (var i = 0; i < this.inputs.length; i++) {
		var node = this.inputs[i];

		// if this node is the bias node...
		if (node.type === NEAT.nodeTypes.BIAS) {

			// feed an input value of 1
			this.feedInput(node.id, 1);
		}
	}
	return this;
};
...
// resets every node's activation (except for the bias node)
NEAT.Network.prototype.reset = function() {
	for (var i = 0; i < this.all.length; i++) {
		var node = this.all[i];
		if (node.type === NEAT.nodeTypes.BIAS) continue;
		node.reset();
	}
	return this;
};
...
NEAT.Network.prototype.feedInputs = function(inputArray) {
	...
	// the index of the bias node
	var biasIndex = 0;

	for (var i = 0; i < this.inputs.length; i++) {
		var node = this.inputs[i];

		// if this node is a bias node...
		if (node.type === NEAT.nodeTypes.BIAS) {
			biasIndex = i;

			/*
			temporarily add a value to the input array so the 
			rest of the inputs align with the correct node
			*/
			inputArray.splice(i, 0, 0);

			// skip over this node, do not input anything to the bias node
			continue;
		}
		if (i >= inputArray.length) return this;
		node.output = inputArray[i];
	}

	// remove the temporarily added number from the inputArray
	inputArray.splice(biasIndex, 1);
	return this;
};

/*
used to feed a specific input node an activation value,
