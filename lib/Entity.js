...
/*
gets the output values of the network, returns them 
in an object with id -> activation key -> value pairs
*/
NEAT.Entity.prototype.getOutputs = function() {
	var outputs = {};

	// for every output node...
	for (var i = 0; i < this.brain.outputs.length; i++) {
		var outputNode = this.brain.outputs[i];

		// add a key value pair to the outputs object
		outputs[outputNode.id] = outputNode.output;
	}
	return outputs;
};
