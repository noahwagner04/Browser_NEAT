...
NEAT.Entity.prototype.feedSensoryInputs = function(inputArray) {
	...
	for (var i = 0; i < inputArray.length; i++) {
		...
		/*
		find the coorisponding input node in the network (we add 1 because we 
		don't want to activate the bias node, whose id is always going to be 0)
		*/
		var inputNode = this.brain.inputs.find(node => node.id === i + 1);

		// if the input node is in the network, activate it
		if(inputNode) {
			inputNode.output = input;
		}
	}
	return this;
};
...
NEAT.Entity.prototype.createChild = function() {
	return new Entity(this.genome.clone().mutate());
};