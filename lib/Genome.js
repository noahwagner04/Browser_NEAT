	...
	*/
	...
	while (!done) {
		...
		for (var i = 0; i < this.genes.length; i++) {
			...
			/*
			if the gene is disabled, don't check if it exists
			remove this check when adding toggle enable and re-enable mutation functions
			as it could result in having duplicate genes

			e.g. say we add a gene connecting nodes 1 and 4, and later we disable it. 
			Next, say we mutate a connection, which happens to be connecting 1 and 4 
			(it passes the existing gene check because we ignore disabled genes). 
			Next, say we end up running the re-enable mutation function, 
			and it re-enables the original 1 to 4 connection. Now we have 
			two 1 - 4 connections, which is not what we want.
			*/
			if(!currentGene.enabled) continue;
